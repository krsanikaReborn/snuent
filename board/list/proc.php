<?php
	...
	If ( $MODE == 'WR' ) {
		...
		$DB->bind_param('P_FILE_NAME',					$FILE_NAME,						_VARCHAR, 				500,				_PARAM_IN);
		$DB->bind_param('P_REALFILE_NAME',			$REALFILE_NAME,				_VARCHAR, 				500,				_PARAM_IN);
	...
	If ( $MODE == 'RR' ) {
		If(isBoardAuth($_SESSION['HOME']['USER_AUTH'], $REPLY_LEVEL_ARRAY) == False) location(2, '권한이 없습니다.', '?id=' . $BID); // 권한체크 - 답변

		$IDX = (!Empty($_POST['idx'])) ? IntVal($_POST['idx']) : Null;

		If (Empty($IDX) || $IDX == 0) {
			$message = '원본글이 이미 삭제되었거나 존재하지 않습니다.';
			$returnUrl = _BOARD_URL_ . "?{$queryString}";
			jsonResponse($message, $returnUrl);
		}
		...
		If($DB->ExecuteProc()) {
			...
			If(!IsSet($Result)) location(2, '원본글이 이미 삭제되었거나 존재하지 않습니다.', "?id={$BID}");

			$parentGrp 	= $Result[0]['GRP'];
			$parentSeq 	= $Result[0]['SEQ'];
			$parentLvl 	= $Result[0]['LVL'];
		}

		// 정렬기준 재정리
		$DB->setProcName('SP_BOARD_SEQ_UPDATE');
		$DB->bind_param('P_GRP',								$parentGrp,							_INT, 						11,				_PARAM_IN);
		$DB->bind_param('P_SEQ',								$parentSeq,							_INT, 						11,				_PARAM_IN);
		If(!$DB->ExecuteProc()) location(2, '댓글 정렬 중 오류가 발생하였습니다. 관리자에게 문의하십시오.', "?id={$BID}");

		$newSeq 		= $parentSeq + 1;
		$newLevel		= $parentLvl + 1;
		$newGrp		= $parentGrp;
		$newUpIdx	= $IDX;
		
		// 댓글의 경우 공지로 등록불가(일반글로 세팅)
		$NOTI = 2;
		...
		If ($DB->ExecuteProc()){
			$Result = $DB->get_fetch_assoc();
			$NEXT_ID = $Result[0]['NEXT_ID'];
		} Else location(2, '신규 게시글 번호 생성 중 오류가 발생하였습니다. 관리자에게 문의하십시오.', "?id={$BID}");
		...
		If ($status == True) location(2, '등록되었습니다.', "?id={$BID}");
		Else location(2, '게시글 등록에 실패하였습니다. 관리자에게 문의하십시오.', "?id={$BID}");
	}
